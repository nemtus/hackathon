...
export const createAggregateCompleteTransactionToCreateAndSetUpNewTeam = async (
  ...
  const description = `CreateAndSetUpNewTeam${adminUserYearTeam.yearId}`;
  ...
  const embeddedAccountAddressRestrictionTransaction =
  ...
  const addressAdditions: UnresolvedAddress[] = [multisigAccount.address];
  ...
  const embeddedAccountAddressRestrictionTransaction2 =
    AccountAddressRestrictionTransaction.create(
      deadline,
      AddressRestrictionFlag.AllowIncomingAddress,
  ...
  const embeddedTransactions = [
    ...
    embeddedAccountAddressRestrictionTransaction2,
  ];
