...
export const createAggregateCompleteTransactionToUpdateFinalVote = async (
  ...
  const description = `UpdateFinalVote${adminUserYearFinalVote.yearId}`;
  ...
  for (const finalVote of adminUserYearFinalVote.votes) {
    ...
    if (finalVote.userId !== userId) {
      logger.debug('finalVote.userId is not matched to userId');
      continue;
    }
    if (
      !(
