...
export const createAggregateCompleteTransactionToCreateNewFinalJudge = async (
  ...
  const description = `CreateNewFinalJudge${adminUserYearFinalJudge.yearId}`;
  ...
  for (const finalJudge of adminUserYearFinalJudge.judges) {
    ...
    if (finalJudge.userId !== userId) {
      logger.debug('finalJudge.userId is not matched to userId');
      continue;
    }
    if (
      !(
